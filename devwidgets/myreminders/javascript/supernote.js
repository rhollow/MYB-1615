...
SuperNote.prototype.pTypes.mousetrack = function(obj, noteID, nextVis, nextAnim){
    with (obj) {
        ...
        if (nextVis && !note.animating && !note.visible) {
            ...
            if (!note.trackTimer) 
                note.trackTimer = setInterval(posString, 50)
        }
        else 
            if (!nextVis && !nextAnim) {
...
SuperNote.prototype.mouseHandler = function(evt, show){
    with (this) {
        ...
        while (elm && elm != rootElm) {
            if (targRE.test(elm.id) || (trigFind && trigRE.test(elm.className))) {
                ...
                if (ref) {
                    ...
                    var note = notes[noteID];
                    if (!note.click || (trigRef != srcElm)) 
                        foundNotes[noteID] = true;
        ...
        if (show == 2) 
            for (var n in notes) {
                if (notes[n].click && notes[n].visible && !foundNotes[n]) 
                    display(n, 0)
            }
    }
};
...
SuperNote.prototype.checkType = function(noteID, nextVis, nextAnim){
    with (this) {
        ...
        if ((/snp-([a-z]+)/).test(note.ref.className)) 
            pType = RegExp.$1;
        if ((/snb-([a-z]+)/).test(note.ref.className)) 
            bType = RegExp.$1;
        if (nextAnim && bType && bTypes[bType] && (bTypes[bType](this, noteID, nextVis) == false)) 
            return false;
        if (pType && pTypes[pType]) 
            pTypes[pType](this, noteID, nextVis, nextAnim);
        return true
    }
};
