...
if (!sakai.composenotification){
    ...
    /**
     * @name sakai.composenotification
     *
     * @class sendmessage
     *
     * @description
    ...
    sakai.composenotification = function(tuid, showSettings) {
            ...
            $(messageFieldMultipleTo).autocomplete(allFriends, {
                ...
                formatItem: function(row){
                    ...
                    if (row.profile.picture && $.parseJSON(row.profile.picture).name) {
                        s = '<img src="/~' + row.profile["rep:userId"] + '/public/profile/' + $.parseJSON(row.profile.picture).name + '" alt="profile icon" width="24" height="24" /> ';
                    }
         ...
         */
        sakai.composenotification.initialise = function(userObj, allowOtherReceivers, insertInId, callback, subject, body) {
